...
export default function Agenda() {
  ...
  const formatDate = (d: Date) => d.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' });
  ...
  return (
    <AppLayout title="Agenda" subtitle={formatDate(selectedDate)}
      actions={
          ...
          {hours.map((hour) => {
            ...
            return (
              <div key={hour} className="flex min-h-[72px]">
                <div className="w-16 lg:w-20 flex-shrink-0 py-3 px-3 text-xs text-muted-foreground font-medium border-r border-border flex items-start">{hour}</div>
                <div className="flex-1 p-2 flex flex-wrap gap-2">
                  {slotAppts.map((appt) => (
                    <motion.div key={appt.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                      className={cn(
                        'flex-1 min-w-[200px] max-w-md rounded-lg p-3 border transition-all',
                        appt.status === 'finalizado' && 'bg-success/5 border-success/20',
                        appt.status === 'em_andamento' && 'bg-warning/5 border-warning/20',
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-1 text-xs text-muted-foreground"><Clock className="w-3 h-3" />{appt.time}</div>
                        <span className="text-xs font-semibold text-primary">R$ {appt.price.toFixed(2)}</span>
                      </div>
                      {appt.levaTraz && (
                        <span className="inline-block mt-2 text-[10px] bg-info/15 text-info px-2 py-0.5 rounded-full">
                      ...
                      {appt.status !== 'finalizado' && appt.status !== 'cancelado' && (
                        <div className="flex gap-2 mt-3 pt-2 border-t border-border/50">
                          <button onClick={() => updateAppointmentStatus(appt.id, nextStatus(appt.status) as any)}
                            className="flex-1 text-[11px] font-medium py-1.5 rounded-md bg-success/15 text-success hover:bg-success/25 transition-colors">
                            {appt.status === 'agendado' ? 'Iniciar' : 'Finalizar'}
                          </button>
                          <button onClick={() => updateAppointmentStatus(appt.id, 'cancelado')}
                            className="text-[11px] font-medium px-3 py-1.5 rounded-md bg-destructive/15 text-destructive hover:bg-destructive/25 transition-colors">
                            Cancelar
                          </button>
                        </div>
                      )}
